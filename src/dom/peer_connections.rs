...
fn render_qr_section(parent: &Element, output: &PeerConnectionsOutput, ctx: &DomCtx) {
    ...
    // set_inner_html parse) is the dominant render cost for this window.
    // The <details> ships collapsed and the payload almost never changes,
    // so defer generation until the user actually opens it — same
    // lazy-on-toggle pattern as the "Scan QR Code" sibling below.
    let qr_content = util::create_element("div");
    qr_content.set_attribute("style", "margin-top:8px").ok();
    {
        let content_ref = qr_content.clone();
        let payload = output.qr_payload.clone();
    ...
    util::append(&qr_details, &scan_details);
    util::append(parent, &qr_details);
}
