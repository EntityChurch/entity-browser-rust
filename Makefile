...
# failure (Stage 1B).
wasm: image
	$(call RUN,trunk build && ./tools/check-dist.sh)
...
wasm-release: image
	$(call RUN,trunk build --release && ./tools/check-dist.sh)
...
wasm-measurement: image
	$(call RUN,trunk build --features measurement)
...
e2e-worker: image
	...
	$(call RUN,trunk build --features demo-apps && ./tools/check-dist.sh)
...
define snapshot_serve_dir
	...
	@echo "==> snapshot fresh SPA → isolated serve dir $(SERVE_DIR) (background dist/ builds can't clobber it)"
	@rm -rf "$(SERVE_DIR)"
	@mkdir -p "$(SERVE_DIR)"
	@cp -a dist/. "$(SERVE_DIR)/"
endef
publish-serve: wasm
	...
	cargo run --quiet --bin entity-browser -- publish $(SERVE_DIR) --live=$(LIVE) $$APPS_FLAG
	@echo ""
...
publish-papers: publish-papers-preflight wasm
	...
	cargo run --quiet --bin entity-browser -- publish $(SERVE_DIR) --ingest=$(PAPERS_INGEST_DIR) $$APPS_FLAG --deployment-config --config-site=$(PAPERS_HOME) --config-profile=$(CONFIG_PROFILE) --live=$(LIVE) $(if $(PREFIX),--prefix=$(PREFIX),) $(if $(IDENTITY_SEED),--identity-seed=$(IDENTITY_SEED),)
	@echo ""
	@if [ -n '$(NO_SERVE)' ]; then \
